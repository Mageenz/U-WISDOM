      ...
      el-collapse-item(title='区县代理商积分奖励及分润设置' name='1')
        el-form
          el-form-item(v-for='item in records1' label='当区域内商家数量达到：')
            el-input(placeholder='数量' v-model='item.maxBusinessAmount' clearable)
            span.ml15 家时，获得的积分分润比例为：
            el-input(placeholder='比例' v-model='item.rewardRatio' clearable)
          el-form-item
...
export default {
  ...
  methods: {
    submit(records) {
      API.basic.editScoreReward(records).then(res => {
        if(res.data.code === '0') {
